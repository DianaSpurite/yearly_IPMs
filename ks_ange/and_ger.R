...
and_ger_long          <- pivot_longer( and_ger, cols = 
...
grow_yrs    <- lapply( 33:71, grow_yr_plots )
...
grow_df    <- grow_df %>% filter(Year>32)
...
gr_mod_yr3 <- lmer( logsize_t1 ~ logsize_t0 + 
                    logsize_t0_2 + logsize_t0_3 + ( logsize_t0 | Year ), 
                    data = grow_df )
...
grow_yrs2    <- lapply( 33:71, grow_yr_plots2 )
...
grow_yrs3    <- lapply( 33:71, grow_yr_plots3 )
...
year_re <- data.frame( coefficient = paste0( "year_", 
...
projected_pop_ns  <- sapply( 1:39, proj_pop ) 
